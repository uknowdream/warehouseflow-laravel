@extends('layouts.app')
@section('title', 'Warehouse')
@section('content')
<div class="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
    <div>
        <h1 class="text-2xl font-bold">Warehouse</h1>
        <p class="text-sm text-slate-500">Kelola gudang, status operasional, dan jumlah lokasi.</p>
    </div>
    @if(auth()->user()?->canManageMasterData())
        <a href="{{ route('warehouses.create') }}" class="rounded-lg bg-slate-950 px-4 py-2 text-white">Tambah Warehouse</a>
    @endif
</div>

<div class="mb-4 grid gap-3 md:grid-cols-3">
    <div class="rounded-lg bg-white p-4 shadow-sm"><div class="text-xs font-semibold uppercase text-slate-500">Total</div><div class="mt-2 text-2xl font-bold">{{ $summary['total'] }}</div></div>
    <div class="rounded-lg bg-white p-4 shadow-sm"><div class="text-xs font-semibold uppercase text-slate-500">Aktif</div><div class="mt-2 text-2xl font-bold text-emerald-700">{{ $summary['active'] }}</div></div>
    <div class="rounded-lg bg-white p-4 shadow-sm"><div class="text-xs font-semibold uppercase text-slate-500">Nonaktif</div><div class="mt-2 text-2xl font-bold text-rose-700">{{ $summary['inactive'] }}</div></div>
</div>

<form method="GET" action="{{ route('warehouses.index') }}" class="mb-4 rounded-lg bg-white p-4 shadow-sm">
    <div class="grid gap-3 md:grid-cols-[1fr_180px_auto]">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari kode, nama, atau alamat"
               class="rounded-lg border-slate-300 shadow-sm focus:border-slate-900 focus:ring-slate-900">
        <select name="status" class="rounded-lg border-slate-300 shadow-sm focus:border-slate-900 focus:ring-slate-900">
            <option value="">Semua status</option>
            <option value="active" @selected($selectedStatus === 'active')>Aktif</option>
            <option value="inactive" @selected($selectedStatus === 'inactive')>Nonaktif</option>
        </select>
        <div class="flex gap-2">
            <button class="rounded-lg bg-slate-950 px-4 py-2 text-white">Filter</button>
            @if($search || $selectedStatus)
                <a href="{{ route('warehouses.index') }}" class="rounded-lg bg-slate-100 px-4 py-2 text-slate-700">Reset</a>
            @endif
        </div>
    </div>
</form>

<div class="overflow-hidden rounded-lg bg-white shadow-sm">
    <table class="w-full min-w-[820px] text-sm">
        <thead class="bg-slate-50">
            <tr><th class="p-3 text-left">Kode</th><th class="p-3 text-left">Nama</th><th class="p-3 text-left">Alamat</th><th class="p-3 text-right">Lokasi</th><th class="p-3 text-left">Status</th><th class="p-3 text-center">Aksi</th></tr>
        </thead>
        <tbody>
        @forelse($warehouses as $warehouse)
            <tr class="border-t">
                <td class="p-3 font-semibold">{{ $warehouse->code }}</td>
                <td class="p-3">{{ $warehouse->name }}</td>
                <td class="p-3">{{ $warehouse->address ?: '-' }}</td>
                <td class="p-3 text-right">{{ $warehouse->locations_count }}</td>
                <td class="p-3">
                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $warehouse->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">
                        {{ $warehouse->is_active ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </td>
                <td class="p-3">
                    <div class="flex items-center justify-center gap-3">
                    @if(auth()->user()?->canManageMasterData())
                        <a class="text-yellow-600" href="{{ route('warehouses.edit', $warehouse) }}">Edit</a>
                        <form method="POST" action="{{ route('warehouses.destroy', $warehouse) }}" onsubmit="return confirm('Hapus warehouse ini?')">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-600">Hapus</button>
                        </form>
                    @else
                        <span class="text-slate-400">Read only</span>
                    @endif
                    </div>
                </td>
            </tr>
        @empty
            <tr class="border-t"><td colspan="6" class="p-6 text-center text-slate-500">Tidak ada warehouse sesuai filter.</td></tr>
        @endforelse
        </tbody>
    </table>
</div>
<div class="mt-4">{{ $warehouses->links() }}</div>
@endsection
